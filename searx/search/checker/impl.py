...
class Checker:
    ...
    @staticmethod
    def call_test(obj, test_description):
        if isinstance(test_description, tuple):
    ...
    @staticmethod
    def call_tests(obj, test_descriptions):
        for test_description in test_descriptions:
            Checker.call_test(obj, test_description)
        ...
        if 'result_container' in test_parameters:
            for rct in rct_list:
                ...
                if not rct.stop_test:
                    Checker.call_tests(rct, test_parameters['result_container'])
        if not stop_test:
            if 'test' in test_parameters:
                checker_tests = CheckerTests(self.test_results, test_name, rct_list)
                Checker.call_tests(checker_tests, test_parameters['test'])
